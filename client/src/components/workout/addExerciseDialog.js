...
function AddExerciseDialog(props) {
    ...
    async function getExercises() {
        const response = await fetch(`${dbUrl}exercises/`);
        if (!response.ok) {
        ...
        const response2 = await fetch(`${dbUrl}exercises/getCustomExercises/${auth.currentUser.uid.toString()}`);
        if (!response.ok) {
            const message = `An error occurred: ${response.statusText}`;
            window.alert(message);
            return;
        }
        setCustomExercises(await response2.json());
    }
    ...
    function addExercise(exerciseId) {
        props.addExerciseFromDialog(exerciseId);
        setOpen(false);
    }
    ...
    async function deleteCustomExercise(id) {
        const requestOptions = {
            ...
            body: JSON.stringify({ "_id": id, "creatorUid": auth.currentUser.uid })
        };

        const response = await fetch(`${dbUrl}exercises/deleteCustomExercise/${auth.currentUser.uid.toString()}`, requestOptions);
    ...
    function addSelectedExercise() {
        if (selectedExerciseId !== null) {
            props.addExerciseFromDialog(selectedExerciseId);
            setOpen(false);
        }
    }
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                <CreateCustomExerciseDialog getExercises={getExercises} />
                <DialogContent dividers>
                    ...
                    <Stack spacing={2} direction="column">
                        ...
                        <Button variant="contained" onClick={() => addSelectedExercise()}>Add selected</Button>
                    </Stack>
                    ...
                    <Box sx={{
                        ...
                        <Stack spacing={4} direction="column">
                            <Typography variant="h4">My custom exercises</Typography>
                            <Box
                                height={400}
                                my={4}
                                display="flex"
                            ...
                            >
                                <Stack spacing={4} direction="column">
                                    {customExercises.map(function (generateTemplate) {
                                        return (
                                            <React.Fragment key={uuidv4()}>
                                                <Typography>
                                                    <Button variant="contained" onClick={() => addExercise(generateTemplate._id)}>Select</Button>
                                                    {' '}
                                                    {generateTemplate.name}
                                                    <Button variant="contained" color="error" onClick={() => deleteCustomExercise(generateTemplate._id)}>Delete</Button>
                                                </Typography>
                                            </React.Fragment>
                                        )
                                    })}
